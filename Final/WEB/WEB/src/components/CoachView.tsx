...
import { CoachingInsight, Task } from '../types.ts';
...
interface CoachViewProps {
  ...
  tasks: Task[];
  userName: string;
  onAddTask?: (task: Omit<Task, 'id' | 'completed' | 'status' | 'failed'>) => void;
}
...
interface Message {
  ...
  theme?: 'clarity' | 'recovery' | 'simplify';
}

export default function CoachView({ insights, tasks, userName, onAddTask }: CoachViewProps) {
  // Goal calibration & alignment mode (세션 유지: localStorage 복원)
  const [partnerMode, setPartnerMode] = useState<'clarity' | 'recovery' | 'simplify'>(
    () => (localStorage.getItem('coach_mode') as 'clarity' | 'recovery' | 'simplify') || 'clarity'
  );
  ...
  // Partner Messages Log (세션 유지: localStorage 복원)
  ...
  const buildWelcome = (mode: 'clarity' | 'recovery' | 'simplify'): Message => {
    let modeText = '';
    if (mode === 'clarity') {
      modeText = `반가워요, ${userName}님! 오늘 꼭 해내고 싶지만 시작하기는 어쩐지 부담스러운 목표가 있으신가요? 마음속에 있는 막연한 생각을 입력해주시면, 아주 실천하기 쉬운 작은 일들로 쪼개드릴게요. 같이 시작해봐요!`;
    } else if (mode === 'recovery') {
      modeText = `요즘 쉼 없이 달리시느라 피로가 쌓이진 않으셨나요? 목표를 끝까지 완주하려면 가장 중요한 건 '내 에너지 지키기'예요. 오늘은 무리한 일정 대신, 뇌에 휴식을 주는 충전 전략을 같이 세워봐요.`;
    } else {
      modeText = `일이 너무 크면 시작하기가 부담스러워져요. 목표를 30분 단위의 작은 행동으로 쪼개서, 가볍게 하나씩 치고 나가는 방법을 안내해 드릴게요.`;
    }
    return {
      sender: 'partner',
      text: modeText,
  ...
  // 환영 메시지 — 처음 진입(저장된 대화 없음)이거나, 사용자가 포커스 모드를 바꿨을 때만
  useEffect(() => {
    if (prevModeRef.current === null) {
      // 최초 마운트 / 탭 재진입 → 저장된 대화가 있으면 유지, 없으면 환영
      prevModeRef.current = partnerMode;
      setMessages(prev => (prev.length === 0 ? [buildWelcome(partnerMode)] : prev));
      return;
    }
    if (prevModeRef.current !== partnerMode) {
      // 사용자가 포커스 모드 전환 → 새 환영으로 리셋
      prevModeRef.current = partnerMode;
      setMessages([buildWelcome(partnerMode)]);
    }
  }, [partnerMode, userName]);

  // 세션 저장 — 대화/모드가 바뀔 때마다 localStorage에 보관
  useEffect(() => {
    localStorage.setItem('coach_messages', JSON.stringify(messages));
  }, [messages]);
  useEffect(() => {
    localStorage.setItem('coach_mode', partnerMode);
  }, [partnerMode]);
    ...
    setTimeout(() => {
      ...
      if (!steps) {
        if (textToSend.includes('피크타임') || textToSend.includes('오전')) {
          ...
          steps = [
            { label: '1순위 집중 격리', duration: '50분', details: '팀 메신저와 알림 배너를 일체 잠금하고 마스터 기획 문서 편집에 완전히 격리 집중', difficulty: '상' },
          ...
          replyText = `${userName}님, 같이 살펴보니 지금 고민 중인 일은 목표가 좀 큰 것 같아요. 기획과 실행을 나눠서 가볍게 시작해볼게요.`;
          steps = [
  ...
  const handleRunCalibration = (e: React.FormEvent) => {
    ...
    const result = {
      ...
      ] as PartnerDecomposedStep[]) : match.steps
    };
  ...
  return (
    <div className="flex-1 flex max-w-[1280px] mx-auto w-full h-[84vh] text-[#1c1c14] relative bg-[#f7f6f2]/50 overflow-hidden font-sans">
      
      {/* 💬 LEFT COLUMN: The Primary 1:1 Conversational Chat Workspace (Always Default & Open) */}
      <div className={`flex-1 flex flex-col bg-white h-full relative overflow-hidden ${showTracker ? 'border-r border-[#e0ddd8]' : ''}`}>
                          ...
                          {msg.suggestedSteps.map((st, sIdx) => (
                            <div key={sIdx} className="relative text-left">
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                                ...
                                <span className="bg-[#f7f6f2] border border-[#e0ddd8] text-[#6b6b58] font-mono font-bold text-[11px] px-1.5 py-0.5 rounded-md block max-w-fit shadow-sm">
                                  {st.duration}
                                </span>
                              </div>
      ...
      {showTracker && (
        <aside className="w-80 md:w-96 border-l border-[#e0ddd8]/75 bg-[#f7f6f2] flex flex-col h-full overflow-y-auto shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.02)] animate-slide-over z-25 shrink-0 text-left">
          ...
          <header className="p-4 bg-white border-b border-[#e0ddd8] flex items-center justify-between gap-3 shrink-0">
            <div className="flex items-center gap-2">
