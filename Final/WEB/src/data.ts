...
import { Task, UserProfile, CoachingInsight, CoachingHistoryItem } from './types.ts';
...
export const IN_FIVE_DAYS_STR = getRelativeDate(5);

// 마감일까지 남은 일수 라벨 (D-day). 마감일이 없으면 null 반환.
export const getDeadlineInfo = (deadline?: string): { text: string; urgent: boolean; overdue: boolean } | null => {
  if (!deadline) return null;
  const today = new Date(TODAY_STR + 'T00:00:00');
  const due = new Date(deadline + 'T00:00:00');
  const diffDays = Math.round((due.getTime() - today.getTime()) / 86400000);
  if (diffDays < 0)  return { text: `D+${Math.abs(diffDays)}`, urgent: true,  overdue: true };
  if (diffDays === 0) return { text: 'D-DAY', urgent: true, overdue: false };
  return { text: `D-${diffDays}`, urgent: diffDays <= 2, overdue: false };
};
