...
import { ViewTab, UserProfile, Task, TaskStatus } from './types.ts';
import { INITIAL_PROFILE, INITIAL_TASKS, INITIAL_INSIGHTS, TODAY_STR } from './data.ts';
...
export default function App() {
  ...
  const [isDarkMode, setIsDarkMode] = useState<boolean>(() => localStorage.getItem('adaptive_dark') === 'true');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('adaptive_dark', String(isDarkMode));
  }, [isDarkMode]);
        ...
        <div className="flex w-full max-w-[1920px] h-full overflow-hidden bg-[#e0ddd8] p-2.5 gap-0 rounded-xl">
          ...
          <div className="flex-grow flex flex-col h-full overflow-hidden bg-[#f7f6f2] rounded-r-lg">
            ...
            <TopAppBar
              ...
              onSearch={(q) => { setGlobalSearch(q); handleSelectTab('todos'); }}
            />
