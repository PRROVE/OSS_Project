...
import { ViewTab, UserProfile, Task } from './types';
...
export default function App() {
  // 첫 방문 여부 — 온보딩 완료 후 다음 로그인은 바로 대시보드로
  const hasOnboarded = localStorage.getItem('adaptive_onboarded') === 'true';
  ...
  // Handlers — 3단계 사이클: 미완료 → 완료 → 실패 → 미완료
  const handleToggleTask = (id: string) => {
    setTasks(prev => prev.map(task => {
      ...
      if (!task.completed && !task.failed) {
        ...
        // 실패 → 미완료 (리셋)
        return { ...task, completed: false, failed: false };
      }
    }));
  };

  // Automated time-out task auditor: checks if current time has passed scheduled slots
  useEffect(() => {
    const checkExpiredTasks = () => {
      ...
      setTasks(prev => {
        ...
        const checked = prev.map(task => {
          // If task is today, incomplete, and not already failed
          if (task.date === TODAY_STR && !task.completed && !task.failed) {
            let targetTime = '';
            
            if (task.timeEnd) {
              targetTime = task.timeEnd;
            } else if (task.timeStart) {
              // If only timeStart is present, assume deadline is 1 hour after start
              const [sh, sm] = task.timeStart.split(':').map(Number);
              if (!isNaN(sh) && !isNaN(sm)) {
                const eh = (sh + 1) % 24;
                targetTime = `${String(eh).padStart(2, '0')}:${String(sm).padStart(2, '0')}`;
              }
            }
            ...
            if (targetTime) {
              ...
              if (!isNaN(h) && !isNaN(m)) {
                ...
                // If current time is past target threshold limit
        ...
        return hasChanges ? checked : prev;
      });
    };
    ...
    const interval = setInterval(checkExpiredTasks, 15000);
  ...
  const handleAddTask = (newTaskFields: Omit<Task, 'id' | 'completed'>) => {
    const newTask: Task = {
      ...newTaskFields,
      id: `task-${Date.now()}`,
      completed: false
  ...
  const handleDeleteTask = (id: string) => {
    // strict partner rule: cannot delete any task once registered! Force mark as failed instead of delete
    setTasks(prev => prev.map(task => {
      if (task.id === id) {
        return { 
          ...task, 
  ...
  const handleSelectTab = (tab: ViewTab) => {
    // 로그인 후 이미 온보딩을 완료했으면 대시보드로 바로 이동
    if (tab === 'onboarding' && hasOnboarded) {
      setTab('dashboard');
    } else {
      setTab(tab);
    }
