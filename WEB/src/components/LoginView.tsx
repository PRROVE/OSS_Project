  ...
  return (
    <div className="w-full h-full flex items-center justify-center p-4 md:p-8">
      <main className="w-full max-w-5xl bg-white rounded-2xl shadow-level-2 flex flex-col md:flex-row overflow-hidden min-h-[620px]">
        ...
        <section className="hidden md:flex md:w-[42%] flex-col justify-between p-10 relative overflow-hidden"
          ...
          <div className="relative z-10 space-y-6 text-left">
            ...
            <div className="space-y-4 pt-4 border-t border-white/10">
              {[
                { Icon: Sparkles, color: '#9a9a86', bg: 'rgba(255,255,255,0.05)', title: '끝까지 가는 단계 분해', desc: '막막한 일을 15분 단위로 슬라이싱' },
        ...
        <section className="flex-1 flex flex-col justify-center p-8 sm:p-12 bg-white">
          <div className="max-w-[340px] w-full mx-auto space-y-5">
            ...
            <div className="hidden md:block text-left">
              <p className="text-[11px] font-semibold text-[#1c1c14] uppercase tracking-wider">Platform Authentication</p>
              <h2 className="text-[18px] font-bold text-[#1c1c14] mt-1">
                {isLogin ? '워크스페이스 로그인' : '무료 파트너십 구축'}
              </h2>
              <p className="text-[12px] text-[#9a9a86] mt-0.5">
                {isLogin ? '등록된 이메일로 안전하게 로그인하세요.' : '30초 만에 완수 가동 환경을 형성합니다.'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                { label: '카카오', node: <span className="w-4 h-4 flex items-center justify-center bg-[#FEE500] text-[#191919] rounded-full font-bold text-[11px]">K</span> },
                { label: '네이버', node: <span className="w-4 h-4 flex items-center justify-center bg-[#03C75A] text-white rounded font-bold text-[11px]">N</span> },
              ].map(({ label, node }) => (
